...
function synogroupuser()
{
	...
	for i in ${userlist}; do
		if [[ "${query}" == "adduser" ]]; then
			...
			synodsmnotify -c SYNO.SDS.3rdParty.${app_name}.Application @administrators ${app_name}:app:title ${app_name}:app:groupuser_error
			exit 1
		fi
	done
	...
	if [[ -z "${userexists}" && "${query}" == "adduser" ]]; then
		...
		synodsmnotify -c SYNO.SDS.3rdParty.${app_name}.Application @administrators ${app_name}:app:title ${app_name}:app:adduser_true
	elif [[ -n "${userexists}" && "${query}" == "adduser" ]]; then
		synodsmnotify -c SYNO.SDS.3rdParty.${app_name}.Application @administrators ${app_name}:app:title ${app_name}:app:adduser_exists
		exit 2
	elif [[ -n "${userexists}" && "${query}" == "deluser" ]]; then
		synogroup --member ${group} ${updatelist[@]}
		synodsmnotify -c SYNO.SDS.3rdParty.${app_name}.Application @administrators ${app_name}:app:title ${app_name}:app:deluser_true
	elif [[ -z "${userexists}" && "${query}" == "deluser" ]]; then
		synodsmnotify -c SYNO.SDS.3rdParty.${app_name}.Application @administrators ${app_name}:app:title ${app_name}:app:deluser_notexist
		exit 3
	fi
